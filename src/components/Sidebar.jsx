...
const Sidebar = ({navlinks, selectedPage, setSelectedPage }) => {
  ...
  return (
    ...
    animate={{ opacity:1, x: selectedPage === name.toLowerCase() ? 0 : -10 }}
    exit={{ opacity: 0,  x: selectedPage !== name.toLowerCase() ? -20 : 10  }}
    transition={{ duration:0.5  }}
    className= {selectedPage===name.toLowerCase()? 'stroke-primary': 'hidden' }
  >
    <Line10px />
  </motion.span>
  </AnimatePresence>
  <AnimatePresence>
    <motion.div
     ...
     exit={{ opacity: 0,  x: selectedPage !== name.toLowerCase() ? -10 : 5  }}
